...
Year_t1 <- 2009:2016
pchange.all2 <- data.frame("Plot" = rep(unique(plants$Plot), 8),"Site" = rep(substr(unique(plants$Plot),1,1),8), "Year_t1" = rep(Year_t1, each =150), "Year_t" = rep(2008:2015, each = 150), "Year_number" = rep(1:8, each = 150), pchange.all) # 8years, 150locations per year across site
...
yy2 <- gsub("_t1", "", yy)
yy2 <-yy2[-c(52)] # to remove name "year"
