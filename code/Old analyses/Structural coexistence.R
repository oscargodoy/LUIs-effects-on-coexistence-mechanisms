...
for(i in 1:dim(combos3)[1]){
  ...
  intrinsic2 <- intrinsic[combos3[i,],4] ## 4 to select the column of site average
...
nd_fd3.2 <- nd_fd3
nd_fd3.2<-within(nd_fd3.2, omega[omega>1] <- 1)
hist(nd_fd3.2$omega)
...
#Which of these triplets coexist due to pure intransitivity
...
#Still need to solve which pairs are intransitive. That pure intransitivity does not occur does not mean
#that they are not intransitive pairs stabilized by niche differences. 
