...
### top50
top50 <- rev(sort(apply(plants[,-c(1:5)], 2, mean,na.rm=T)))[1:50]
top50.short <- c("Poa_tri", "Poa_pra", "Alo_pra", "Dac_glo", "Tri_rep", "Tar_off", "Lol_per", "Arr_ela", 
                 ...
                 "Ant_odo", "Bra_pin", "Car_hir", "Pha_aru", "Ver_cha", "Rum_ace", "Fes_ovi", "Phl_pra",
...
names(plants3)[1:50] <- top50.short
...
#If they are as happen in our case then, we are ok to follow. https://stats.stackexchange.com/questions/97929/lmer-model-fails-to-converge
for(i in 1:51){
...
site.mat <- matrix(nrow=length(yy)-2, ncol=length(yy)-2) #It has one dimension less because the species Pha_aru does not occur across sites
intrinsic.site.lui <- matrix(nrow=length(yy)-2, ncol=length(3))
